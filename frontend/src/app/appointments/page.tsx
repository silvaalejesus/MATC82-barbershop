...
interface Appointment {
  id: string;
  // O '?' permite que não quebre se o backend não mandar o objeto completo
...
export default function AppointmentsPage() {
  ...
  // Estado local substitui o atom de agendamentos estáticos
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [loading, setLoading] = useState(true);

  // 1. Busca os agendamentos ao carregar a página
  useEffect(() => {
    if (!user) {
      router.push("/login");
      return;
  ...
  const handleCancel = async (id: string) => {
    ...
    try {
      ...
      // Atualiza a lista localmente para 'cancelled' sem precisar recarregar tudo
  ...
  // Se não tiver usuário, não renderiza nada enquanto redireciona
  if (!user) return null;
            ...
            <Card key={appointment.id} className="flex flex-col">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="flex items-center gap-1 mt-1">
                  ...
                  {/* Garante que a data seja exibida corretamente */}
